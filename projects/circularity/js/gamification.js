...
var Gamification = (function () {
    ...
    init: function (gameReferences) {
      ...
      Multiplayer.init({
        onConnected: function () {
          ...
          // Defer UI updates to avoid blocking the connection handler
          setTimeout(function () {
          ...
          if (remotePlayer && data) {
            ...
            if (opponentScore) {
              opponentScore.textContent =
                "Opponent: Size " + Math.floor(data.radius);
            }
          }
        },
      });
  ...
  function createPlayer() {
    var playerColor =
      Multiplayer.isMultiplayer && !Multiplayer.isHost ? "#0080FF" : "#00FF00";
  ...
  function updateScore() {
    ...
    if (scoreElement) {
      scoreElement.textContent =
        "Score: " + score + " | Size: " + Math.floor(player.radius);
    }
  }
  ...
  function resetGame() {
    ...
    gameStarted = false;
    gameMode = "single";
    document.getElementById("instructions").style.display = gamificationUnlocked
      ? "block"
      : "none";
  ...
  function setupUIEventListeners() {
    document
      ...
      .addEventListener("click", function () {
        gameMode = "single";
        document.getElementById("singlePlayerInstructions").style.display =
          "block";
        document.getElementById("multiplayerInstructions").style.display =
          "none";
  ...
  function endMultiplayerGame(reason) {
    ...
    var remoteSize = Multiplayer.remotePlayerData
      ? Multiplayer.remotePlayerData.radius
      : 15;
